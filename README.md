...
**M3 идёт** — то, ради чего плагин станет пригоден для микса. После разворота
позиционирования это уже не «музыкальность», а обязательная часть
([docs/STRATEGY.md](docs/STRATEGY.md) §9). Сделаны диффузия хвоста (четыре алл-пасса
Шрёдера в петле) и фильтры хвоста (однополюсные, на входе кольца — каждый круг темнее).
Осталось: модуляция времени, ducking, ping-pong, tempo sync.
