...
**Решение сессии 23: первый macOS-релиз выходит pkg без подписи.** На вопрос о членстве
в Apple Developer Program пользователь ответил «не сейчас». Значит, на странице загрузки
нужна инструкция про System Settings → Privacy & Security → «Open Anyway», а окно, которое
увидит пользователь, надо снять хотя бы раз до релиза — сейчас оно не снималось (выше).
Раздел 5 остаётся непроверенным до покупки членства.
...
- [#42](ISSUES.md) — **сделано в сессии 23:** `THIRD-PARTY-NOTICES.txt` в `Contents/Resources/`
  обоих бандлов, кладёт CMake до подписи. Проверка после упаковки:
  `codesign --verify --strict` на `build/pkg/root/VST3/…` и `…/Components/…`.
- [#49](ISSUES.md) — имя **Berg, решено в сессии 23**. Идентификаторы — `com.Berg.MidiDelay`, код
  производителя `Berg`, пакет `com.Berg.MidiDelay.pkg` — после первого релиза
  не меняются: по ним хосты находят плагин в сохранённых проектах. Домен не куплен.
- Версия в `CMakeLists.txt` — одна на бандлы и pkg. Сейчас `0.1.0`.
- Standalone собирается, но в pkg не входит нарочно: продукт — плагин.
- Минимальная macOS — **11.0** (`CMAKE_OSX_DEPLOYMENT_TARGET`, сессия 25). До этого бандлы
  несли `minos 26.0` и не грузились на старших системах. Перед упаковкой проверить:
  `vtool -show-build` на обоих бандлах. На живой macOS 11–15 не запускалось.
- Страница загрузки — `site/midi-delay/`, [#49](ISSUES.md). Не опубликована: ждёт форму Kit.
